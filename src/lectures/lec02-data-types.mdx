...
Be careful, a variable that refers to `None` and an undefined variable 
(that is not bound to any value) are different situations. 
If the variable `y` has not been introduced yet 
(or has been deleted with `del y`), evaluating the expression `y is None` 
raises an error
...
Invoking a Python function can also have **side effects**, for example:
...
Looking up a missing key in a dictionary raises a `KeyError`.
...
Fortunately, `dict.get(key)` returns `None` in this case instead of raising an error.
...
You can compute the remainder and quotient of the Euclidean division of two integers.
...
Floating-point numbers can represent non-integer numeric values, 
like $e$ or $\pi$:
...
finite precision representation of real numbers ; 
any computation may also induce additional errors:
```python
>>> 0.1 + 0.2
0.30000000000000004
>>> 0.1 + 0.2 == 0.3
